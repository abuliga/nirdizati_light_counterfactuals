...
def dice_impressed(CONF, predictive_model, cf_df, encoder, df, query_instance, method, optimization, heuristic, support,
                 timestamp_col_name,model_path,neighborhood_size,case_id=None,random_seed=None,diversity_weight=None,sparsity_weight=None,proximity_weight=None,
    ...
    if predictive_model.model_type == ClassificationMethods.LSTM.value:
        ...
        dice_query_instance = dice_ml.Dice(d, m, method='genetic')
    else:
        ...
        dice_query_instance = dice_ml.Dice(d, m, optimization)
        ...
        while total_traces <  k:
            ...
            if method == 'genetic_conformance':
                dice_result_flip = dice_query_instance.generate_counterfactuals(x,encoder=encoder, desired_class=int(1-predicted_outcome),
                                                                           ...
                                                                           heuristic=heuristic,random_seed=random_seed,features_to_vary=features_to_vary)
    ...
    filename_cf = path_cf + 'cf_%s_%s_dice_%s_diversity_%s_%s.csv' % (dataset, feature_selection, case_id,diversity_weight,method)
    ...
    try:
        ...
        filename_cf_pos = path_cf + 'cf_%s_%s_dice_%s_pos_diversity_%s_%s.csv' % (dataset, feature_selection, case_id,diversity_weight,method)
        filename_cf_neg = path_cf + 'cf_%s_%s_dice_%s_neg_diversity_%s_%s.csv' % (dataset, feature_selection, case_id,diversity_weight,method)
        #event_log = convert_to_dataframe(event_log)
